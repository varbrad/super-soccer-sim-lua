...
function club_history:set_team()
	...
	for i=iter, 1, -1 do
		...
		if data.promoted or data.relegated then	bar.rects[2] = { x = bar.rects[1].x + bar.rects[1].w, y = 0, w = bar.w - bar.rects[1].w, h =g.skin.bars.h, color = col, alpha = g.skin.bars.alpha } end

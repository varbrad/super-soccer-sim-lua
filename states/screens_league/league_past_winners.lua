...
function league_past_winners:set()
	...
	for i=iter, 1, -1 do
		...
		bar.labels[2] = { text = team1.long_name, x = bar.images[1].x + g.skin.margin * 2 + bar.images[1].w, y = g.skin.bars.ty, font = g.skin.bars.font[2], color = c1 }
		bar.labels[3] = { text = team2.long_name, x = bar.images[2].x + g.skin.margin * 2 + bar.images[2].w, y = g.skin.bars.ty, font = g.skin.bars.font[2], color = c2 }
		bar.labels[4] = { text = team3.long_name, x = bar.images[3].x + g.skin.margin * 2 + bar.images[3].w, y = g.skin.bars.ty, font = g.skin.bars.font[2], color = c3 }
		...
		btn1.on_release = function(btn) g.database.vars.view.team_id = team1.id; g.state.switch(g.states.club_overview) end
		btn2.on_enter = function(btn) bar.labels[3].underline = true end
		btn2.on_exit = function(btn) bar.labels[3].underline = false end
		btn2.on_release = function(btn) g.database.vars.view.team_id = team2.id; g.state.switch(g.states.club_overview) end
		btn3.on_enter = function(btn) bar.labels[4].underline = true end
		btn3.on_exit = function(btn) bar.labels[4].underline = false end
		btn3.on_release = function(btn) g.database.vars.view.team_id = team3.id; g.state.switch(g.states.club_overview) end
